...
InputParameters RBEConstraint::validParams() {
  InputParameters params = NodalConstraint::validParams();
  params.addClassDescription(
      "Constrains secondary node to move as a linear combination of primary "
      "nodes. "
      "Implementation of a Rigid Body Element Constraint");
  params.addRequiredParam<BoundaryName>(
      "primary_node_set", "The boundary ID associated with the primary nodes.");
  params.addRequiredParam<BoundaryName>(
      "secondary_node_set",
  ...
  params.addRequiredParam<Real>("primary_size",
                                "The number of nodes in the primary node set.");
  return params;
}
...
Real RBEConstraint::computeQpResidual(Moose::ConstraintType type) {
  /*
  ...
  split up such that the final secondary residual resembles the above
  expression.
  */
...
Real RBEConstraint::computeQpJacobian(Moose::ConstraintJacobianType type) {
  unsigned int primary_size = _primary_size;
