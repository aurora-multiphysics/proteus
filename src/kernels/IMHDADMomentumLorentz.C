...
InputParameters IMHDADMomentumLorentz::validParams() {
  ...
  params.addRequiredCoupledVar(
      "currentDensity",
      "The variable representing the electric current density");
  params.addParam<FunctionName>("magneticFieldFunction",
                                "A function defining a vectorValue method that "
                                "describes the imposed magnetic field");
  params.addParam<FunctionName>("magneticFieldFunction_x", "0",
                                "A function that describes the x-component of "
                                "the imposed magnetic field");
  params.addParam<FunctionName>("magneticFieldFunction_y", "0",
                                "A function that describes the y-component of "
                                "the imposed magnetic field");
  params.addParam<FunctionName>("magneticFieldFunction_z", "0",
                                "A function that describes the z-component of "
                                "the imposed magnetic field");
...
IMHDADMomentumLorentz::IMHDADMomentumLorentz(const InputParameters &parameters)
    : ADVectorKernelValue(parameters),
      ...
      _magnetic_field(isParamValid("magneticFieldFunction")
                          ? &getFunction("magneticFieldFunction")
                          : nullptr),
  ...
  if (_magnetic_field && parameters.isParamSetByUser("_magnetic_field_x"))
    paramError("magneticFieldFunction_x",
               "The 'magneticFieldFunction' and 'magneticFieldFunction_x' "
               "parameters cannot both be set.");
  if (_magnetic_field && parameters.isParamSetByUser("_magnetic_field_y"))
    paramError("magneticFieldFunction_y",
               "The 'magneticFieldFunction' and 'magneticFieldFunction_y' "
               "parameters cannot both be set.");
  if (_magnetic_field && parameters.isParamSetByUser("_magnetic_field_z"))
    paramError("magneticFieldFunction_z",
               "The 'magneticFieldFunction' and 'magneticFieldFunction_z' "
               "parameters cannot both be set.");
}

ADRealVectorValue IMHDADMomentumLorentz::precomputeQpResidual() {
  if (_magnetic_field)
    return -_current_density[_qp].cross(
        _magnetic_field->vectorValue(_t, _q_point[_qp]));
  else
    return -_current_density[_qp].cross(
        RealVectorValue(_magnetic_field_x.value(_t, _q_point[_qp]),
