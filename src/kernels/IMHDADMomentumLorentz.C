...
IMHDADMomentumLorentz::validParams()
{
  ...
  params.addClassDescription("The Lorentz force term ($\\vec{J} \\times \\vec{B}_0$), "
                             "with the weak form of ($\\phi_u, \\vec{J} \\times \\vec{B}_0$)."
                             "The Jacobian is computed using automatic differentiation.");
  params.addRequiredCoupledVar("currentDensity",
  ...
  params.addParam<FunctionName>(
      "magneticFieldFunction_x",
      "0",
      "A function that describes the x-component of the imposed magnetic field");
  params.addParam<FunctionName>(
      "magneticFieldFunction_y",
      "0",
      "A function that describes the y-component of the imposed magnetic field");
  params.addParam<FunctionName>(
      "magneticFieldFunction_z",
      "0",
      "A function that describes the z-component of the imposed magnetic field");
...
IMHDADMomentumLorentz::IMHDADMomentumLorentz(const InputParameters & parameters)
  : ADVectorKernelValue(parameters),
    ...
    _magnetic_field(isParamValid("magneticFieldFunction") ? &getFunction("magneticFieldFunction")
                                                          : nullptr),
    ...
    _magnetic_field_z(getFunction("magneticFieldFunction_z"))
{
  if (_magnetic_field && parameters.isParamSetByUser("_magnetic_field_x"))
    paramError(
        "magneticFieldFunction_x",
        "The 'magneticFieldFunction' and 'magneticFieldFunction_x' parameters cannot both be set.");
  if (_magnetic_field && parameters.isParamSetByUser("_magnetic_field_y"))
    paramError(
        "magneticFieldFunction_y",
        "The 'magneticFieldFunction' and 'magneticFieldFunction_y' parameters cannot both be set.");
  if (_magnetic_field && parameters.isParamSetByUser("_magnetic_field_z"))
    paramError(
        "magneticFieldFunction_z",
        "The 'magneticFieldFunction' and 'magneticFieldFunction_z' parameters cannot both be set.");
}
