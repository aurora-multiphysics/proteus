...
InputParameters IMHDADCurrentUxB::validParams() {
  InputParameters params = ADVectorKernelValue::validParams();
  params.addClassDescription(
      "The electric current velocity source term ($-\\vec{U} \\times "
      "\\vec{B}$), "
...
ADRealVectorValue IMHDADCurrentUxB::precomputeQpResidual() {
  if (_magnetic_field)
    return -_velocity[_qp].cross(
