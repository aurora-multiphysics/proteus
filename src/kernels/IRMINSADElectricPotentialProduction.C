...
InputParameters IRMINSADElectricPotentialProduction::validParams() {
  InputParameters params = ADKernelGrad::validParams();
  params.addClassDescription(
      "The Electric Potential production term ($\\nabla \\cdot (\\vec{u} "
      "\\times \\vec{B}_0)), "
      "with the weak form of $(\\nabla \\phi_i, -(\\vec{u} \\times "
      "\\vec{B}_0)). "
...
ADRealVectorValue IRMINSADElectricPotentialProduction::precomputeQpResidual() {
