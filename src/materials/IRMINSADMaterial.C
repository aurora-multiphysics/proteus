...
InputParameters IRMINSADMaterial::validParams() {
  ...
  params.addParam<MaterialPropertyName>("conductivity", "conductivity",
                                        "The name of the conductivity"); // IRM
  return params;
}

IRMINSADMaterial::IRMINSADMaterial(const InputParameters &parameters)
    : Material(parameters), _velocity(adCoupledVectorValue("velocity")),
      ...
      _conductivity(getADMaterialProperty<Real>("conductivity")), // IRM
      _velocity_dot(nullptr),
      _mass_strong_residual(declareADProperty<Real>("mass_strong_residual")),
      _advective_strong_residual(
          declareADProperty<RealVectorValue>("advective_strong_residual")),
      _lorentz_electrostatic_strong_residual(declareADProperty<RealVectorValue>(
          "lorentz_electrostatic_strong_residual")), // IRM
      _lorentz_flow_strong_residual(declareADProperty<RealVectorValue>(
          "lorentz_flow_strong_residual")), // IRM
      // We have to declare the below strong residuals for integrity check
      // purposes even though we may not compute them. This may incur some
      // unnecessary cost for a non-sparse derivative container since when the
      // properties are resized the entire non-sparse derivative containers will
      // be initialized to zero
      _td_strong_residual(
          declareADProperty<RealVectorValue>("td_strong_residual")),
      _gravity_strong_residual(
          declareADProperty<RealVectorValue>("gravity_strong_residual")),
      _boussinesq_strong_residual(
          declareADProperty<RealVectorValue>("boussinesq_strong_residual")),
      ...
      _relative_velocity(
          declareADProperty<RealVectorValue>("relative_velocity")),
  ...
  if (!_fe_problem.hasUserObject("ins_ad_object_tracker")) {
    ...
    _fe_problem.addUserObject("INSADObjectTracker", "ins_ad_object_tracker",
                              tracker_params);
  }

  // Bypass the UserObjectInterface method because it requires a UserObjectName
  // param which we don't need
  _object_tracker =
      &_fe_problem.getUserObject<INSADObjectTracker>("ins_ad_object_tracker");
  const_cast<INSADObjectTracker *>(_object_tracker)
      ->addBlockIDs(this->blockIDs());
}

void IRMINSADMaterial::subdomainSetup() {
  if ((_has_transient =
           _object_tracker->get<bool>("has_transient", _current_subdomain_id)))
  ...
  if ((_has_boussinesq = _object_tracker->get<bool>("has_boussinesq",
    ...
    // Material property retrieval through MaterialPropertyInterface APIs can
    // only happen during object contruction because we're going to check for
    // material property dependency resolution. So we have to go through
    // MaterialData here. We already performed the material property requests
    // through the MaterialPropertyInterface APIs in the INSAD kernels, so we
    // should be safe for dependencies
    _boussinesq_alpha = &_material_data.getProperty<Real, true>(
        _object_tracker->get<MaterialPropertyName>("alpha",
                                                   _current_subdomain_id),
        0, *this);
    auto &temp_var = _subproblem.getStandardVariable(
        _tid, _object_tracker->get<std::string>("temperature",
                                                _current_subdomain_id));
    addMooseVariableDependency(&temp_var);
    _temperature = &temp_var.adSln();
    _ref_temp = &_material_data.getProperty<Real, false>(
        _object_tracker->get<MaterialPropertyName>("ref_temp",
                                                   _current_subdomain_id),
        0, *this);
  } else {
  ...
  _has_gravity =
      _object_tracker->get<bool>("has_gravity", _current_subdomain_id);
  if (_has_gravity || _has_boussinesq)
    _gravity_vector =
        _object_tracker->get<RealVectorValue>("gravity", _current_subdomain_id);
  else
    _gravity_vector = 0;

  // Setup data for Arbitrary Lagrangian Eulerian (ALE) simulations in which the
  // simulation domain is displacing. We will need to subtract the mesh velocity
  // from the velocity solution in order to get the correct material velocity
  // for the momentum convection term.
  if ((_has_advected_mesh = _object_tracker->get<bool>(
           "has_advected_mesh", _current_subdomain_id))) {
    auto &disp_x = _subproblem.getStandardVariable(
        _tid,
        _object_tracker->get<VariableName>("disp_x", _current_subdomain_id));
    ...
    if (_object_tracker->isTrackerParamValid("disp_z", _current_subdomain_id)) {
      auto &disp_z = _subproblem.getStandardVariable(
          _tid,
          _object_tracker->get<VariableName>("disp_z", _current_subdomain_id));
  ...
  _viscous_form = static_cast<NS::ViscousForm>(int(
      _object_tracker->get<MooseEnum>("viscous_form", _current_subdomain_id)));

  if ((_has_coupled_force = _object_tracker->get<bool>(
           "has_coupled_force", _current_subdomain_id))) {
    ...
    if (_object_tracker->isTrackerParamValid("coupled_force_var",
      ...
      for (const auto &var_name : var_names)
        _coupled_force_var.push_back(
            &_subproblem.getVectorVariable(_tid, var_name).adSln());
    }
    ...
    if (_object_tracker->isTrackerParamValid("coupled_force_vector_function",
      ...
      for (const auto &func_name : func_names)
        _coupled_force_vector_function.push_back(
            &_fe_problem.getFunction(func_name, _tid));
    }
  }
}

void IRMINSADMaterial::computeQpProperties() {
  _mass_strong_residual[_qp] = -_grad_velocity[_qp].tr();
  if (_coord_sys == Moose::COORD_RZ)
    // Subtract u_r / r
    _mass_strong_residual[_qp] -=
        _velocity[_qp](_rz_radial_coord) /
        (_use_displaced_mesh ? _ad_q_point[_qp](_rz_radial_coord)
                             : _q_point[_qp](_rz_radial_coord));

  _advective_strong_residual[_qp] =
      _rho[_qp] * _grad_velocity[_qp] * _velocity[_qp];
  ...
  _lorentz_flow_strong_residual[_qp] =
      -_conductivity[_qp] * UxB.cross(_magnetic_field[_qp]); // IRM
  ...
  if (_has_boussinesq)
    _boussinesq_strong_residual[_qp] =
        (*_boussinesq_alpha)[_qp] * _gravity_vector * _rho[_qp] *
  ...
  if (_has_advected_mesh) {
    ...
    _advected_mesh_strong_residual[_qp] =
        -_rho[_qp] * _grad_velocity[_qp] * _mesh_velocity[_qp];
  }

  if (_has_coupled_force) {
    _coupled_force_strong_residual[_qp] = 0;
    mooseAssert(
        !(_coupled_force_var.empty() && _coupled_force_vector_function.empty()),
        "Either the coupled force var or the coupled force vector function "
        "must be "
        "non-empty in 'IRMINSADMaterial'");
    for (const auto *var : _coupled_force_var) {
      mooseAssert(var, "null coupled variable in IRMINSADMaterial");
      _coupled_force_strong_residual[_qp] -= (*var)[_qp];
    }
    for (const auto *fn : _coupled_force_vector_function) {
      mooseAssert(fn, "null coupled function in IRMINSADMaterial");
      _coupled_force_strong_residual[_qp] -= fn->vectorValue(_t, _q_point[_qp]);
    }
  }

  // // Future Addition
  // _mms_function_strong_residual[_qp] = -RealVectorValue(_x_vel_fn.value(_t,
  // _q_point[_qp]),
  //                                                       _y_vel_fn.value(_t,
  //                                                       _q_point[_qp]),
  //                                                       _z_vel_fn.value(_t,
